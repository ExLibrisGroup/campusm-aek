...
let attrsO = {};
_.forEach(attrs,(attr)=>{ attrsO[attr] = true; });


var pickHtmlProps = function(props,extra = {}) {
  if (_.isArray(extra)) { extra = {include:extra}; }
  let include = _.get(extra,"include",[]);
  let exclude = _.get(extra,"exclude",[]);
  return _.pickBy(props,(val,name)=>{
    if(name.match(/^(data|aria)\-/)) { return true; }
    return (attrsO[name] || (include.indexOf(name) > -1)) && (exclude.indexOf(name) === -1);
  });
};

module.exports = pickHtmlProps;
