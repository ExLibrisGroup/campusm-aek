/* eslint-disable no-multi-assign,no-use-before-define,no-useless-escape,no-plusplus,no-cond-assign,eqeqeq,one-var,prefer-template */
...
ArgParser.prototype = {
  ...
  command: function(name) {
    ...
    var chain = {
      ...
      option: function(n, spec) {
        command.specs[n] = spec;
        return chain;
      },
  ...
  parse: function(argv) {
    ...
    /* parse the args */
    var that = this;
    args.reduce(function(ar, val) {
      /* positional */
      if (ar.isValue) {
        positionals.push(ar.value);
      }
      else if (ar.chars) {
        var last = ar.chars.pop();

        /* -cfv */
        (ar.chars).forEach(function(ch) {
          that.setOption(options, ch, true);
        });

        const thatOpt = that.opt(last);

        /* -v key */
        if (!thatOpt.flag && !thatOpt.isImplicit) {
          if (val.isValue) {
        ...
        if (value === undefined) {
          /* --key value */
          const thatOpt = that.opt(ar.full);
          if (!thatOpt.flag && !thatOpt.isImplicit) {
            if (val.isValue) {
              that.setOption(options, ar.full, val.value);
              return Arg();
            }

            that.print(`'--${that.opt(ar.full).name || ar.full}'`
                + ` expects a value\n\n${that.getUsage()}`, 1);
        ...
        that.setOption(options, ar.full, value);
      }
      return val;
    });
  ...
  getUsage: function() {
    ...
    if (options.length) {
      ...
      var longst = options.reduce(function(max, opt) {
        return opt.string.length > max && !opt.hidden ? opt.string.length : max;
      }, 0);

      options.forEach(function(opt) {
        if (!opt.hidden) {
          str += `   ${opt.string}${spaces(longst - opt.string.length)}   `;
...
var Opt = function(opt) {
  ...
  opt = _(opt).extend({
    ...
    matches: function(arg) {
      return opt.full == arg || opt.abbr == arg || opt.position == arg //eslint-disable-line eqeqeq
        || opt.name == arg || (opt.list && arg >= opt.position); //eslint-disable-line eqeqeq
    }
  });
  return opt;
};
