...
<html lang="en">
<head>
    <title>Code coverage report for react/styles/site.var.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">react/styles</a> site.var.js
    </h1>
    <div class='clearfix'>
...
999
1000
1001</td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import {extend,get,isString,find} from "lodash";
&nbsp;
import {getAekConfig} from "../../aek-config";
import { 
  unitMath, 
  makeReadable, 
  darken, 
...
&nbsp;
&nbsp;
export default styleMemoize(function() {
&nbsp;
  const emSize = "14px";
&nbsp;
  function getSize(px,unit = "rem") {
    if (isString(px) &amp;&amp; sizes[px]) {
      px = Math.round(unitMath(sizes[px],"*",emSize));
    }
    return `${unitMath(px,"/",emSize)}${unit}`;
  }
&nbsp;
  function getAbsoluteSize(px) {
    return getSize(px,"em");
  }
&nbsp;
  function getRelativeSize(px) {
    return getSize(px,"em");
  }
&nbsp;
  function getBaseColor(variation) {
    return isString(variation) 
      ? getAekConfig(`style.${variation}Color`) 
      : get(variation,"color");
  }
&nbsp;
  function getColor(variation) {
    return getBaseColor(variation) || get(defaultColors,variation);
  }
&nbsp;
  function getLightColor(variation) {
    const varColor = isString(variation) 
      ? getAekConfig(`style.${variation}LightColor`) 
      : <span class="branch-1 cbranch-no" title="branch not covered" >get(variation,"lightColor");</span>
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (varColor) { <span class="cstat-no" title="statement not covered" >return varColor; </span>}
&nbsp;
    const baseColor = getBaseColor(variation);
    if (!baseColor &amp;&amp; defaultLightColors[variation]) {
      return defaultLightColors[variation];
    }
    return lightenRelative(getColor(variation),15);
  }
&nbsp;
  function getBackgroundColor(variation) {
    const varColor = isString(variation) 
      ? getAekConfig(`style.${variation}BackgroundColor`)
      : <span class="branch-1 cbranch-no" title="branch not covered" >get(variation,"backgroundColor");</span>
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (varColor) { <span class="cstat-no" title="statement not covered" >return varColor; </span>}
&nbsp;
    const baseColor = getBaseColor(variation);
    
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!baseColor &amp;&amp; defaultBackgroundColors[variation]) {
<span class="cstat-no" title="statement not covered" >      return defaultBackgroundColors[variation];</span>
    }
    return lightenRelative(getColor(variation),95,0.99);
  }
&nbsp;
  function getInvertedBackgroundColor(variation) {
    return makeReadable(getColor(variation));
  }
&nbsp;
  function getTextColor(variation) {
    return makeReadable(getColor(variation));
  }
&nbsp;
  function getBorderColor(variation) {
    return getTextColor(variation);
  }
&nbsp;
  function <span class="fstat-no" title="function not covered" >getHeaderColor(</span>variation) {
<span class="cstat-no" title="statement not covered" >    return darken(getTextColor(variation),5);</span>
  }
&nbsp;
  function getHoverColor(variation) {
    return saturate(darkenOrLighten(getColor(variation), 5), 10, "relative");
  }
&nbsp;
  function getFocusColor(variation) {
    return saturate(darken(getColor(variation), 8), 20, "relative");
  }
&nbsp;
  function getDownColor(variation) {
    return darken(getColor(variation), 10);
  }
&nbsp;
  function getActiveColor(variation) {
    return saturate(darken(getColor(variation), 5), 15, "relative");
  }
&nbsp;
  function getLightHoverColor(variation) {
    return saturate(darken(getLightColor(variation), 5), 10, "relative");
  }
&nbsp;
  function getLightFocusColor(variation) {
    return saturate(darken(getLightColor(variation), 8), 20, "relative");
  }
&nbsp;
  function getLightActiveColor(variation) {
    return saturate(darken(getLightColor(variation), 5), 15, "relative");
  }
&nbsp;
  function getLightDownColor(variation) {
    return darken(getLightColor(variation), 10);
  }
&nbsp;
  const absoluteBorderRadius = getAbsoluteSize(4);
  const loaderSize = getRelativeSize("big");
  const loaderOffset = `-${unitMath(loaderSize,"/",2)}`;
  const fullBlack = "#000000";
  const offWhite = "#F9FAFB";
  const darkWhite = "#F3F4F5";
  const midWhite = "#DCDDDE";
  const white = "#FFFFFF";
  const black = getColor("black");
  const textColor = "rgba(0, 0, 0, 0.87)";
  const inputColor = textColor;
  const borderColor = "rgba(34, 36, 38, 0.15)";
  const minContrast = 2.5;
  const linkColor = "#4183C4";
  const transparentBlack = "rgba(0, 0, 0, 0.05)";
&nbsp;
  function <span class="fstat-no" title="function not covered" >bestTextColor(</span>bgColor,colors = <span class="branch-0 cbranch-no" title="branch not covered" >[white,textColor])</span> {
<span class="cstat-no" title="statement not covered" >    return bestContrast(bgColor,colors);</span>
  }
&nbsp;
  function firstReadableColor(bgColor,colors = [white,textColor], mContrast = minContrast) {
    return find(colors,col=&gt;getContrast(solidifyOpacity(col,bgColor),bgColor) &gt;= mContrast);
  }
&nbsp;
&nbsp;
  return extend({
...
&nbsp;
    ...
    pageFont: getAekConfig("style.pageFont",
...
&nbsp;
    ...
    relativeBorderRadius: getRelativeSize(14),
    ...
    inputBackground: "#fff",
    inputVerticalPadding: getRelativeSize(11),
    inputHorizontalPadding: getRelativeSize(14),
...
&nbsp;
    ...
    linkColor,
    // @linkUnderline       : none;
    linkHoverColor: darkenRelative(saturate(linkColor, 20), 15),
...
&nbsp;
    ...
    subtleShadow: `0px 1px 2px 0 ${borderColor}`,
&nbsp;
    // @floatingShadow:
